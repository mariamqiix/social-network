...
export default function page(params: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProfileData(data);
        setIsActive(
          data.is_user_profile ||
            data.user_status == "Accepted" ||
            data.user_profile_type == "Public"
        );
      } catch (error) {
  ...
  const handleUnfollowRequest = () => {
    fetch("http://localhost:8080/user/responds/requestToUnfollow", {
      method: "POST",
      credentials: "include",
      body: JSON.stringify({ user_id: profileData?.user.id, reaction: "" }),
    }).then((res) => {
  ...
  return (
    <div className="Container">
      <Metadata
        seoTitle={"Friendz | " + (profileData?.user?.username ?? "")}
        seoDescription="The next gen social network"
      />
      {/* Profile Header */}
        ...
        <div className="profile-info">
          <img
            src={
              ...
              profileData.user.image_url != undefined
                ? `data:image/jpeg;base64,${profileData?.user?.image_url}`
                : ""
            }
              ...
              {isActive && (
                <div>
                  ...
                  <p className="profile-desc">
                    {calculateAge(profileData?.user.DateOfBirth)} Years
                  </p>
                ...
                {!profileData?.is_user_profile ? (
                  <div className="button-container">
                    {profileData?.user_status === "" ? (
                      <button
                        className="followBtn requestBtn"
                      ...
                      <button
                        className="unfollowBtn requestBtn"
                      ...
                      profileData?.user_status === "Pending" && (
                        <button
                          className="pendingBtn requestBtn"
                          onClick={() => handleUnfollowRequest()}
                        >
                          <span>Requested</span>
                        </button>
                  ...
                  <div className="button-container">
                    <select
                      ...
                      value={profileData.user_profile_type}
                      onChange={(e) => {
                        console.log(e.target.value);
                        fetch("http://localhost:8080/user/changePrivacy/", {
                          method: "POST",
                          credentials: "include",
                          body: JSON.stringify({
                            user_id: user?.id,
          ...
          <ul className="member-list">
            {follows != null &&
              follows.map((member) => (
                ...
                >
                  <a href={`/profile/${member.id}`} className="member-item">
                    <img
