...
export default function SearchBar() {
              ...
              credentials: 'include', method: "POST"
                ...
                response.json().then((json) => {
                    ...
                    json.forEach((userJson:any) => {
                        newUsers.push({id:userJson["ID"],username: userJson["Username"]});
                    });
                    setUsers(newUsers);
                })
            });
    ...
    return <div className="w-100 p-2">
        ...
        <div className={(isOpen ? "" : "d-none ") + "position-fixed z-1 p-3 w-100 text-bg-light"}>
            {pages.filter((item => item.includes(searchTerms))).map((item, index) => <p key={index}><Link className="btn btn-link" href={"/" + (item == "posts" ? "" : item)}>{item}</Link></p>)}
            ...
            {users.filter((item => item.username.includes(searchTerms))).map((u) => <p key={u.id+"-user"}><Link className="btn btn-link" href={"/profile/" + u.id}>{u.username}</Link></p>)}
        </div>
    </div>;
}